...
class MusicRecommender:
    ...
    def __init__(self):
        ...
        # Conector Externo (Suspendido en 2do Plano por restricciones Premium de Spotify)
        self.spotify = None
    def preparar_dataset(self, ruta_csv=None):
        print("\n⚙️  Inicializando el Motor de Inteligencia Artificial...")
        if ruta_csv:
            ...
            print(f"✅ ¡Cargadas {len(self.df):,} canciones únicas! Usando {len(self.caracteristicas)} dimensiones de ADN.")
        else:
            datos_mock = {
            ...
            print("✅ Dataset de prueba cargado (Práctica).")
        ...
        print("✅ Caché de Identidades Rápidas (NER) ensamblada en Memoria.\n")
    ...
    def recomendar(self, input_usuario, modo='contenido', exportar=False, override_type=None, override_index=None):
        ...
        if modo == 'contenido':
            match_artista = self.df['artist'].str.lower() == input_limpio
            match_cancion = self.df['track_name'].str.lower() == input_limpio
            
            if not match_artista.any() and not match_cancion.any():
                match_artista = self.df['artist'].str.contains(input_limpio, case=False, na=False, regex=False)
                match_cancion = self.df['track_name'].str.contains(input_limpio, case=False, na=False, regex=False)
            if not match_artista.any() and not match_cancion.any():
            ...
            if match_artista.any():
                print(f"\n✨ ¡Artista Encontrado! Analizando la discografía de: '{input_usuario}'...")
                canciones_origen = self.df[match_artista]
                df_resultados = self.df[~match_artista].copy()
            else:
                print(f"\n✨ ¡Canción Encontrada! Analizando el track...")
            ...
            if 'track_genre' in self.df.columns:
                generos_artista = canciones_origen['track_genre'].dropna().unique()
                if len(generos_artista) > 0:
                    print(f"   [!] Mismo ADN detectado en Géneros: {', '.join(generos_artista)[:50]}... Filtrando ruidos.")
                    df_resultados = df_resultados[df_resultados['track_genre'].isin(generos_artista)]
            ...
            # CÁLCULO HÍBRIDO: 65% ADN Acústico / 35% Fama Global
            df_resultados['match_percent'] = df_resultados['distancia'].apply(
                lambda d: min(100.0, max(1.0, 100.0 - (d * 12.0)))
            )
            df_resultados['hybrid_score'] = (df_resultados['match_percent'] * 0.50) + (df_resultados['popularity'] * 0.50)
            ...
            print(f"\n🧠 Analizando matriz psicológica y acústica de: '{input_usuario}'...")
            try:
            ...
            if artistas_detectados or generos_detectados:
                ...
                if generos_detectados: print(f"       ▶ Géneros: {', '.join(generos_detectados).title()}")
            ...
            print(f"   [!] Cruzando {len(lista_canciones)} pistas con nuestra Base de Datos Kaggle para clonar su ADN...")
            ...
            # Match heurístico: track coincidente o artista coincidente
            filtro_local = self.df['track_name'].str.lower().isin(nombres_buscados) | self.df['artist'].str.lower().isin(artistas_buscados)
            df_match = self.df[filtro_local]
            
            if df_match.empty:
               print("   ❌ Ninguna de las bandas de esa playlist existe en los registros masivos de Kaggle. Imposible clonar.")
               return None
               
            print(f"   ✅ ¡Genética Acústica equivalente extraída vía {len(df_match)} canciones locales compatibles!")
            ...
            top_cercanas = df_resultados.sort_values(by='distancia').head(5000)
            recomendaciones = top_cercanas.sort_values(by='popularity', ascending=False).head(cantidad_resultados)
